...
async function loadDashboard() {
  ...
  document.getElementById('hero-greeting').textContent = `${g}, ${(currentUserData?.nome||'').split(' ')[0]}!`;
  document.getElementById('hero-date').textContent = new Date().toLocaleDateString('pt-BR',{weekday:'long',day:'numeric',month:'long'});

  const q = query(collection(db,'routes'), where('cobradorId','==',currentUser.uid), orderBy('criadoEm','desc'));
  onSnapshot(q, snap => {
...
function renderRouteList(id, routes, emptyMsg) {
  ...
  el.innerHTML = routes.length
    ...
    : `<div class="routes-empty"><p>${emptyMsg}</p></div>`;
}

function routeCardHTML(r) {
  const ini = Holidays.formatDateBR(r.dataInicio);
  const fim = Holidays.formatDateBR(r.dataFim);
  const per = (r.dataFim && r.dataFim !== r.dataInicio) ? `${ini} → ${fim}` : ini;
  const lbl = {planejada:'Planejada', em_andamento:'Em andamento', concluida:'Concluída'};
  const cls = {planejada:'badge-planejada', em_andamento:'badge-em_andamento', concluida:'badge-concluida'};
  return `
    <div class="route-card status-${r.status}" onclick="openRoute('${r.id}')">
      <div class="route-card-top">
        <span class="route-card-name">${r.nome||'Sem nome'}</span>
        <span class="route-status-badge ${cls[r.status]||''}">${lbl[r.status]||r.status}</span>
      </div>
      <div class="route-card-meta">
        <span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="3" y1="10" x2="21" y2="10"/></svg> ${per}</span>
        ${r.totalStops?`<span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg> ${r.totalStops} parada(s)</span>`:''}
...
window.onDateChange = async function () {
  ...
  el.innerHTML = '';
  if (!s || !e) return;
  if (e < s) { el.innerHTML = `<div class="date-alert date-alert-info">⚠️ Data de fim deve ser igual ou posterior ao início.</div>`; return; }
  el.innerHTML = `<div class="date-alert date-alert-info"><span>Verificando feriados...</span></div>`;
...
window.calcularRota = async function () {
  if (pendingStops.length < 2) { showToast('Adicione pelo menos 2 endereços.', 'warning'); return; }
  showToast('Calculando rota...', 'info');
  try {
    const coords = pendingStops.map(s => `${s.longitude},${s.latitude}`).join(';');
    const res = await fetch(`${OSRM_URL}/${coords}?overview=false&steps=false`);
    const data = await res.json();
    ...
    showToast(`Rota: ${dist} · ${tempo}`, 'success');
    return { dist, tempo };
  } catch (e) {
...
window.salvarRota = async function () {
  ...
  if (!nome)             { showToast('Digite um nome para a rota.','warning'); return; }
  if (!inicio)           { showToast('Selecione o período.','warning'); return; }
  if (!pendingStops.length){ showToast('Adicione pelo menos um endereço.','warning'); return; }

  const alertas = (holidayAnalysis?.alertas||[]).filter(a => a.type !== 'info');
  if (alertas.length) {
    showAlertModal(alertas, ok => { if (ok) doSalvarRota(nome, inicio, fim); });
  } else {
    await doSalvarRota(nome, inicio, fim);
  }
};
...
async function renderDetalheRota() {
  ...
  const ini = Holidays.formatDateBR(currentRoute.dataInicio);
  const fim = Holidays.formatDateBR(currentRoute.dataFim);
...
function renderDetalheStops(stops) {
  ...
  Object.entries(byDay).forEach(([date, dayStops]) => {
    ...
    const dataBR  = Holidays.formatDateBR(date);
...
window.recalcularRota = async function () {
  if (!currentRoute?.stops?.length) return;
  pendingStops = currentRoute.stops;
  const result = await calcularRota();
  if (result && currentRoute.id) {
    await updateDoc(doc(db,'routes',currentRoute.id), {
      distanciaTotal: result.dist, tempoTotal: result.tempo, atualizadoEm: serverTimestamp()
    });
    document.getElementById('meta-distancia').textContent = result.dist;
    document.getElementById('meta-tempo').textContent     = result.tempo;
  }
};
...
function initMapRota() {
  ...
  if (!el) return;
  if (mapRota) { mapRota.invalidateSize(); return; }
...
function updateMapRotaStops() {
  ...
  if (coords.length>1) {
    // Desenha rota via OSRM
    const osrmCoords = stops.map(s=>`${s.longitude},${s.latitude}`).join(';');
    fetch(`${OSRM_URL}/${osrmCoords}?overview=full&geometries=geojson`)
      .then(r=>r.json())
      .then(data => {
        if (data.code==='Ok') {
          const geo = data.routes[0].geometry;
          routeLayer = L.geoJSON(geo, { style:{color:'#1A56E8',weight:4,opacity:0.8} }).addTo(mapRota);
        }
      }).catch(()=>{
        // Fallback: linha reta
        routeLayer = L.polyline(coords, {color:'#1A56E8',weight:4,opacity:0.7,dashArray:'8,4'}).addTo(mapRota);
      });
  }
